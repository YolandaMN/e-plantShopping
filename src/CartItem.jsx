...
const CartItem = ({ onContinueShopping }) => {
  ...
  const calculateTotalAmount = () => {
    ...
    cart.forEach(item => {
      ...
      total += quantity * cost; // Add subtotal to total
    });
  ...
  const calculateTotalCost = (item) => {
    const costPerItem = parseFloat(item.cost.substring(1)); // Remove "$" and convert to number
  return (costPerItem * item.quantity).toFixed(2); // Multiply by quantity and format to 2 decimals
  };
